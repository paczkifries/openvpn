    ...
    else if (streq(p[0], "allow-compression") && p[1] && !p[2])
    {
        ...
        if (streq(p[1], "no"))
        {
            ...
            if (comp_non_stub_enabled(&options->comp)) {
                ...
                /*
                 * we could manipulate the algorithm to a stub/no variant but
                 * the only scenario that the previous message does not abort
                 * the program is when pushed from a server and the server
                 * probably has a very confused configuration, so keep whatever
                 * we have
                 */
            }
        }
